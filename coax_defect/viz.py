...
def plot_overview(problem, V, title=None, output_path=None):
    ...
    if labels is not None:
        ax.contourf(X, Y, (labels == 1).astype(float), levels=[0.5, 1.5], colors=['#5da5ff'], alpha=0.2)
        ax.contourf(X, Y, (labels == 2).astype(float), levels=[0.5, 1.5], colors=['#ffcc00'], alpha=0.4)
    ...
    ax = axs[1]
    Em_plot = np.clip(Em, 0.01, np.nanmax(Em))
    im = ax.imshow(
        Em_plot,
        origin='lower',
        extent=extent,
        aspect='equal',
        norm=LogNorm(vmin=max(np.nanmin(Em_plot), 0.01), vmax=np.nanmax(Em_plot)),
        cmap='inferno',
    )
    ...
    ax = axs[2]
    ax.contourf(X, Y, V, levels=28, cmap='viridis', alpha=0.7)
    ...
    Exu[mask_e] = np.nan
    Eyu[mask_e] = np.nan
    ax.streamplot(xu, yu, Exu, Eyu, density=1.2, color='k', linewidth=0.9)
...
def plot_zoom_near_defect(problem, V, radius_factor=4.0, output_path=None, title=None):
    ...
    fig.colorbar(c, ax=ax, shrink=0.9)
    ax = axs[1]
    Em_plot = np.clip(Ems, 0.1, np.nanmax(Ems))
    im = ax.pcolormesh(Xs, Ys, Em_plot, shading='auto', cmap='inferno', norm=LogNorm(vmin=0.1, vmax=np.nanmax(Em_plot)))
...
def plot_radial_diagnostics(problem, V, phi_deg=0.0, num_samples=400, output_path=None, title=None):
    ...
    r = np.linspace(max(Rin * 1.001, Rin + 1e-06), Rout * 0.999, num_samples)
    xs = cx + r * np.cos(phi)
    ys = cy + r * np.sin(phi)
