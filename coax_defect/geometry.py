...
def make_grid(nx, ny, outer_radius):
    ...
    return {'x': x, 'y': y, 'X': X, 'Y': Y, 'nx': int(nx), 'ny': int(ny), 'L': L}

def build_problem(voltage, inner_radius, outer_radius, eps_r, defect_type='none', defect_radius=0.0, defect_center_x=None, defect_center_y=None, defect_epsr=None, nx=400, ny=400):
    ...
    meta = {
        ...
        'eps_r': float(eps_r),
        'defect_type': str(dt),
        'defect_type_code': float(defect_type_code),
...
def solve_potential(problem, max_iter=20000, tol=1e-06, omega=1.6, progress=False, progress_every=250):
    ny = int(problem['ny'])
    nx = int(problem['nx'])
    ...
    for i in range(max_iter):
        ...
        if progress and (i % int(progress_every) == 0 or i == max_iter - 1):
            print(f'iter {i:6d}  max_delta={max_delta:.3e}')
        if max_delta < tol:
            break
    return V
